...
create_mainWin (void)
{
  ...
  g_object_set_data (G_OBJECT (mainWin), "mainWin", mainWin);
  gtk_window_set_title (GTK_WINDOW (mainWin), _("XML Viewer"));
  gtk_window_set_default_size (GTK_WINDOW (mainWin), 640, 480);

  vbox1 = gtk_box_new (FALSE, 0);
  g_object_ref (vbox1);
//  g_object_set_data_full (G_OBJECT (mainWin), "vbox1", vbox1,
...
create_aboutBox (void)
{
  ...
  //gtk_signal_connect_object (G_OBJECT (button), "clicked",
...
#endif /* NEED_GNOMESUPPORT_H */
  g_object_set_data (G_OBJECT (aboutBox), "aboutBox", aboutBox);
